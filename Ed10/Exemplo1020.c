...
ref_int_Matrix readMatrixFromFile(chars fileName, ref_int_Matrix matrix)
{
    ...
    if (arq)
    {
        ...
        else
        {
            ...
            {
                // ler guardar valores em arranjo
                for (x = 0; x <= matrix->rows; x = x + 1)
                {
                    for (y = 0; x <= matrix->columns; y = y + 1)
                    {
                        // ler valor
...
void method_16(void)
{
    ...
    matrix = readMatrixFromFile("DADOS1.TXT", matrix);
...
void method_17(void)
{
    ...
    // teste para ver se todos sao zeros
    resp = matrixZero(matrix);

    // teste se for verdadeiro ou falso
    if (resp)
    {
    ...
    // desaclocar espaco do descritor
    free_int_Matrix(matrix);
...
bool matrixCompare(ref_int_Matrix matrix1, ref_int_Matrix matrix2)
{
    ...
    {
        for (int x = 0; x < matrix1->rows; x = x + 1)
        {
            for (int y = 0; y < matrix1->columns; y = y + 1)
            {
        ...
        if (cont == (matrix1->rows * matrix1->columns))
        {
            return TRUE;
        }
    }
...
void method_18(void)
{
    ...
    // alocacao do descritor
    ref_int_Matrix matrix1 = (ref_int_Matrix)malloc(sizeof(int_Matrix));
    ref_int_Matrix matrix2 = (ref_int_Matrix)malloc(sizeof(int_Matrix));

    // leitura do arquivo
    matrix1 = readMatrixFromFile("DADOS.TXT", matrix1);
    matrix1 = readMatrixFromFile("DADOS.TXT", matrix1);

    // comparacao de dados
    resp = matrixCompare(matrix1, matrix2);

    // teste se sao iguais ou nao
    if (resp)
    {
...
int matrixAdd(ref_int_Matrix matrix1, ref_int_Matrix matrix2)
{
    int result = 0;
    if (matrix1->rows == matrix2->rows && matrix2->columns == matrix1->columns)
    {
        for (int x = 0; x < matrix1->rows; x = x + 1)
        {
            for (int y = 0; y < matrix1->columns; y = y + 1)
            {
                result = result + (matrix1->data[x][y] + matrix2->data[x][y]);
            }
        }
    }
    else
...
void method_19(void)
{
    ...
    // leitura do arquivo
    readMatrixFromFile("DADOS.TXT", matrix1);
    readMatrixFromFile("DADOS1.TXT", matrix2);

    // soma das matrizes
    result = matrixAdd(matrix1, matrix2);

    // saida de dados
    printf("A soma dos valores e: %d", result);
...
*/
int matrixProduct(ref_int_Matrix matrix1, ref_int_Matrix matrix2)
{
    int result = -1;

    if (matrix1->rows == matrix2->rows && matrix1->columns == matrix2->columns)
    {
        for (int x = 2; x < matrix1->rows; x = x + 1)
        {
            for (int y = 2; y < matrix1->columns; y = y + 1)
            {
                result = result + (matrix1->data[x][y] * matrix2->data[x][y]);
            }
        }
...
void method_20(void)
{
    ...
    // alocacao e declaracao do descritor
    ref_int_Matrix matrix1 = (ref_int_Matrix)malloc(sizeof(int_Matrix));
    ref_int_Matrix matrix2 = (ref_int_Matrix)malloc(sizeof(int_Matrix));
    ...
    result = matrixProduct(matrix1, matrix2);

    // reste se houve a multiplicacao
    if (result >= 0)
    {
        printf("o produto de duas matrizes e: %d", result);
    }
    else
    {
        printf("nao houve a multiplicacao.");
    }
...
int main(void)
{
    ...
    {
        // identifcar
        IO_id("EXEMPLO1020 - Programa - v2.0");
