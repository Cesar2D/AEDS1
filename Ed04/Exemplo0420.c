...
/**
Funcao para determinar se caractere e' maiuscula.
@return valor para contagem
@param x - valor a ser testado
*/
int contador(chars x)
{
    // variavel local
    int result = 0, tamanho = 0, y = 0;
...
/**
Funcao para determinar se caractere e' maiuscula.
@return valor para contagem e exibicao
@param x - valor a ser testado
*/
int contExib(chars x)
{
    // variavel local
    int result = 0, tamanho = 0, y = 0;

    // obter o tamanho da cadeia de caracteres
    tamanho = strlen(x);

    // percorrer toda a cadeia de caracteres
    for (y = 0; y < tamanho; y = y + 1)
    {
        // teste se for maiuscula
        if (maiuscula(x[y]))
        {
            // teste se for maior que 'K'
            if (x[y] > 'K')
            { // exibir e contar a palavra
                IO_printf("\n%d. [%c]", y, x[y]);
                result = result + 1;
            }
        }
...
/**
Funcao para determinar se caractere e' maiuscula ou minuscula maior que 'k' e 'K'.
@return valor para contagem e exibicao
@param x - valor a ser testado
*/
int contExibMaiMin(chars x)
{
    // variavel local
    int result = 0, tamanho = 0, y = 0;
    ...
    {
        // teste se for maiuscula
        if (maiuscula(x[y]) || minuscula(x[y]))
        {
            // teste se for maior que 'K'
            if (x[y] > 'K' && x[y] < 'Z' || x[y] > 'k')
            { // exibir e contar a palavra
                IO_printf("\n%d. [%c]", y, x[y]);
                result = result + 1;
            }
...
int exiNaoAlf(chars x)
{
    ...
    // pecorrer toda a palavra
    for (y = 0; y < tamanho; y = y + 1)
    {
        if (!(maiuscula(x[y]) || minuscula(x[y]) || digito(x[y])))
        {
            IO_printf("\n%d. [%c]", y, x[y]);
        } // end if
    }     // end for
...
{
    // variavel local
    int result = 0, tamanho = 0, y = 0, cont = 0;
...
void method_12(void)
{
    ...
    // conversao para inteiro ( tamanho da palavra)
    tamanho = strlen(palavra);

    // para percorrer toda a palavra
    for (y = 0; y < tamanho; y = y + 1)
    {
        // teste se e maiuscula
        if (maiuscula(palavra[y]))
        {
            // teste para saber se e maior que 'K'
            if (palavra[y] > 'K')
            {
                IO_printf("\n%d. [%c]", y, palavra[y]);
                cont = cont + 1;
...
{   
    ...
    // entradas
    n = IO_readint("Digite o numero de palavras: ");

    for (y = 0; y < n; y = y + 1)
    {
        palavra = IO_readstring("\nDigite uma palavra: ");

        cont = cont + exiAlf(palavra);

    } // end for

    IO_printf("\n%s%d", "O numero de Alfanumerico e: ", cont);
...
int main(void)
{
    ...
    {
        // identifcar
        IO_id("EXEMPLO0420 - Programa - v2.0");
