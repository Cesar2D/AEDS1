...
void method_14a(int n, chars fileName, int result, double div)
{
    FILE *arq = fopen(fileName, "at");
    if (n > 0)
    {
        ...
        method_14a(n - 1, fileName, result = result * 7, div);
...
{
    int n = 0;
    FILE *arq = fopen("ARQ_4.TXT", "wt");
    fclose(arq);

    n = IO_readint("Digite o numero de vezes: ");

    method_14a(n, "ARQ_4.TXT", 1, 0.0);
...
void method_15a(int n, chars fileName, double val)
{
...
void method_15(void)
{
    ...
    // criar o arquivo
    FILE *arq = fopen("ARQ_5.TXT", "wt");
    fclose(arq);

    n = IO_readint("Digite o numero de vezes: ");
    m = IO_readdouble("Digite um valor: ");

    method_15a(n, "ARQ_5.TXT", m);
...
double method_16a(int n, chars fileName, double resultado, int base)
{
    int temp = 0;
    if (n > 0)
    {
        ...
        resultado = resultado + method_16a(n - 1, fileName, resultado, base);
...
void method_16(void)
{
    ...
    // criar para depois usar
    FILE *arq = fopen("ARQ_6.TXT", "wt");
    fclose(arq);

    // entradas
    n = IO_readint("Digite o numero de vezes: ");
    m = IO_readint("Digite o valor da base: ");

    // chamada do metodo
    method_16a(n, "ARQ_6.TXT", 1, m);
...
void method_17a(int n, chars fileNameEsq, chars fileNameLei, double temp)
{
    FILE *arqEsc = fopen(fileNameEsq, "wt");
    FILE *arqLei = fopen(fileNameLei, "rt");
    int y = 0;

    fscanf(arqLei, "%lf", &temp);

    while (!feof(arqLei) && y <= n)
    {
        // printar o valor no arquivo
        fprintf(arqEsc, "%lf\n", temp);
        // tentar ler
        fscanf(arqLei, "%lf", &temp);
...
void method_17(void)
{
    ...
    FILE *arqEsc = fopen("ARQ_7.TXT", "wt");
    FILE *arqLei = fopen("ARQ_4.TXT", "rt");
    fclose(arqEsc);
    fclose(arqLei);

    n = IO_readint("Digite o numero de vezes: ");

    method_17a(n, "ARQ_7.TXT", "ARQ_4.txt", 1.0);
...
int method_18a(int n, chars fileName)
{
...
{
    return ('A' <= carac && 'Z' >= carac);
}

void method_19a(chars fileNameLei, chars fileNameEsc)
{
    FILE *arqLei = fopen(fileNameLei, "rt");
    FILE *arqEsc = fopen(fileNameEsc, "at");
    char carac = 'a';

    // IO_fscanf(arqLei, "%c", &carac);
    if (arqLei == NULL)
    {
...
void method_19(void)
{
    method_19a("ARQ_9a.TXT", "ARQ_9.TXT");
...
bool digMaiQ5(char carac)
{
    return('5' <= carac && carac <= '9');
}

void method_20a(chars fileNaLei, chars fileNaEsc)
{
    FILE * arqLei = fopen(fileNaLei, "rt");
    FILE * arqEsc = fopen(fileNaEsc, "wt");
    char dig = '0';
    ...
    else
    {
        while(!feof(arqLei))
        {
            IO_fscanf(arqLei, "%c", &dig);
            // printf("%c", dig);
            
            if(digMaiQ5(dig))
            {
                fprintf(arqEsc, "%c", dig);
            }
        }
    fclose(arqLei);
    fclose(arqEsc);
...
void method_20(void)
{
    method_20a("ARQ_10a.TXT", "ARQ_10.TXT");
...
int main(void)
{
    ...
    {
        // identifcar
        IO_id("EXEMPLO0520 - Programa - v2.0");
